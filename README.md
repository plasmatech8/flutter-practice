...
We can used `CachedVideoPlayer` or `VideoPlayer` as the video player.

We will stack a `InkWell` widget on top of the `CachedVideoPlayer` to allow
tap to pause.

A circular loading icon will show while the video is not loaded (which is quite
short because we are using caching instead of loading full video).

![](docs/2020-07-24-22-08-16.png)
